            ...
            <form  action="{{ route('admin.film.update',['id' => $film->id]) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="filmname" value="{{ old('filmname',$film->filmname)}}" class="form-control" placeholder="Nhập tên phim">
                    </div>
                    <div class="form-group">
                      <label>Hình ảnh cũ</label>
                      <img src="{{ asset('upload') }}/{{$film->image}} " width="100%" height="225">                       
                      <input type="file" name='image' value="{{ old('image',$film->image)}}" class="form-control">                
                    </div>                    
                    <div class="form-group">
                        <label>Video giới thiệu</label>
                        <input type="text" name="trailer" value="{{ old('trailer',$film->trailer)}}" class="form-control" placeholder="Nhập đường dẫn">
                    </div>                    
                        ...
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <div class="input-group-text">                                              
                                    <input type="checkbox" {{in_array($item->id,$arr_cate_film_edit) ? 'checked' : ''}} value="{{ $item ->id }}" name="category_id[]">                         
                                </div>
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" value="{{ old('start_day',$film->start_day)}}" class="form-control" data-inputmask-alias="datetime" name='start_day' data-inputmask-inputformat="dd/mm/yyyy" data-mask="" inputmode="numeric">
                        </div>
                        <!-- /.input group -->
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" value="{{ old('director',$film->director)}}" name="director" class="form-control" placeholder="Nhập tên đạo diễn">
                    </div>
                    <div class="form-group">
                      <label>Quốc gia</label>
                      <input type="text" name="country" value="{{ old('country',$film->country)}}" required class="form-control" placeholder="Nhập tên quốc gia">
                    </div>
                    <div class="form-group">
                      <label>Nhà sản xuất</label>
                      <input type="text" name="NSX" value="{{ old('NSX',$film->NSX)}}" required class="form-control" placeholder="Nhập tên nhà sản xuất">
                    </div>
                    <div class="form-group">
                        <label>Dàn diễn viên</label>
                        <input type="text" name="castes" value="{{ old('castes',$film->castes)}}" required class="form-control" placeholder="Nhập tên dàn diễn viên">
                    </div>

                    <div class="form-group">
                        <label>Thời lượng</label>
                        <input type="number" value="{{ old('time_film',$film->time_film)}}" name="time_film" class="form-control" placeholder="Nhập thời lượng (phút)">
                    </div>
                    <div class="form-group">
                      <label>Mô tả</label>
                      <textarea type="text" id="ten" name="description" class="form-control" rows="3" placeholder="Nhập mô tả của phim">{{ old('description',$film->description)}}
                      </textarea>
